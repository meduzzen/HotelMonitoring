...
class CameraProcessor:
    ...
    def process_frame(self, detector: YOLO) -> np.ndarray | None:
        ...
        if self.frame_count % self.config.detection_interval == 0:
            ...
            # Update tracker
            self.last_tracks = self.tracker.update_tracks(
                self.last_detection, frame=frame
            )
            ...
            if hasattr(self, "last_decoded_frame"):
                self.last_tracks = self.tracker.update_tracks(
                    self.last_detection, frame=self.last_decoded_frame
                )
    ...
    def _draw_track_annotation(
        self, frame: np.ndarray, l: int, t: int, r: int, b: int, global_id: str
    ) -> None:
